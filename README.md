...
1. **❤️ Bookmarked Builds & Playlists Manager (`CollectionsModal.jsx`)**:
   - Save favorite builds with heart bookmark buttons on cards or inside project detail modals. Organize saved builds into custom named playlists (e.g. "Top Favorites", "UI Masterpieces"), export JSON backups, and copy formatted Markdown summaries to clipboard.
2. **🎮 Live App Interactive Sandbox Embed (`SandboxEmbed.jsx`)**:
...
3. **🏷️ Portfolio JSON & PDF Exporter (`PortfolioExport.jsx`)**:
...
4. **💻 Code Snippet Architecture Inspector (`CodeInspector.jsx`)**:
...
5. **🌙 Theme Switcher & Particle Canvas (`ThemeSwitcher.jsx`, `ParticleCanvas.jsx`)**:
...
6. **🎛️ Side-by-Side Build Comparison Matrix (`CompareModal.jsx`)**:
...
7. **📇 Framer Motion Animated Timeline View (`TimelineView.jsx`)**:
